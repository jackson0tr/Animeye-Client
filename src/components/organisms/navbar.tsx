...
import { FaUser, FaSun, FaHome, FaImage, FaInfoCircle, FaPhone, FaCog, FaBoxOpen, FaQuestionCircle, FaSignOutAlt } from "react-icons/fa";
...
export function Navbar() {
    ...
    return (
        <>
            <motion.nav className="sticky top-0 z-50 w-full bg-[var(--background)] border-b border-gray-200">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div className="md:hidden flex items-center flex-1 justify-start px-2">
                            ...
                            <div className="md:hidden flex gap-4 items-center text-[var(--pink)]">
                                <IoMdPerson size={20} className="cursor-pointer hover:text-[var(--secondary)]" />
                                <MdFavorite size={20} className="cursor-pointer hover:text-[var(--secondary)]" />
                                <FaCartShopping size={20} className="cursor-pointer hover:text-[var(--secondary)]" />
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-5 flex flex-row gap-18">
                    {/* {isOpen ? <FiX size={24} onClick={() => setIsOpen(false)} /> : <FiMenu onClick={() => setIsOpen(true)} className="text-[var(--pink)]" size={24} />} */}
                    {isOpen ? <></> : <FiMenu onClick={() => setIsOpen(true)} className="text-[var(--pink)]" size={24} />}
                        ...
                        >
                            <div className="flex flex-col space-y-4">
                                {sidebarItems.map((item, index) => {
                                    ...
                                    const prevIsNotLogout = sidebarItems[index - 1]?.label !== "Log/Sign out";
                                    ...
                                    return (
                                        <motion.div key={item.label} whileHover={{ x: 10 }}>
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    <span className={`text-lg ${isLogout ? "text-[var(--secondary)]" : "text-[var(--pink)]"}`}>
                                                        {item.icon}
                                                    </span>
                                                    <span className={`${isLogout ? "!text-[var(--secondary)]" : "!text-[var(--pink)]"}`}>
                                                        {item.label}
                                                    </span>
                                                </div>
                                                ...
                                                {item.label === "Sun Mode" && (
                                                    <label className="relative inline-flex items-center cursor-pointer ml-2">
                                                        <input
                                                            type="checkbox"
                                                            checked={typeof window !== 'undefined' && localStorage.getItem("theme") === "light"}
